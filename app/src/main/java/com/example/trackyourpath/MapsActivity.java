...
public class MapsActivity extends AppCompatActivity implements OnMapReadyCallback , GoogleMap.OnMarkerClickListener {
    ...
    public void onMapReady(GoogleMap googleMap) {
            ...
            if(jsonArrayOldHIstory.length()>0)
            {
                ...
                for(int i=0;i<jsonArrayOldHIstory.length();i++)
                {
                    try {
                        ...
                        if(i==0)
                        {
                          mMap.addMarker(new MarkerOptions().position(b[i]).title("Starting of Day").icon(BitmapDescriptorFactory.fromResource(R.drawable.location_maps)).snippet(date1));//defaultMarker(BitmapDescriptorFactory.HUE_YELLOW)));//icon(BitmapDescriptorFactory.fromResource(R.drawable.first));
                        }
                        else if(i==(jsonArrayOldHIstory.length()-1))
                        {
                           mMap.addMarker(new MarkerOptions().position(b[i]).title("End of Day").icon(BitmapDescriptorFactory.fromResource(R.drawable.location_maps)).snippet(date1));
                        }
        ...
        //mMap.addMarker(new MarkerOptions().position(Anupam).title("Marker in Surat"));
