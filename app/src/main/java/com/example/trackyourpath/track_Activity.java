...
public class track_Activity extends FragmentActivity implements OnMapReadyCallback, GoogleMap.OnMarkerClickListener {
    ...
    boolean first_time;
    RelativeLayout start1,btn1,getmap1;
    TextView Start1,Btn1,Getmap1;
    Calendar c;
                ...
                if(jsonArrayOldHIstory.length()>0)
                {
                    ...
                    for(int i=0;i<jsonArrayOldHIstory.length();i++)
                    {
                        try {
                            ...
                            mMap.addMarker(new MarkerOptions().position(a).title("Travel Date :").snippet(Day1 +"/"+ Month1 +"/"+ Year1).icon(BitmapDescriptorFactory.fromResource(R.drawable.flag)));
